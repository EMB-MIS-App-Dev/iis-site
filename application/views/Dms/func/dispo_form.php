			...
			<table border="1" id="tbroute">
				...
				<?php
					foreach ($trans_log as $key => $value) {
						echo '<tr class="tc tblr">
							<td>'.$value['sender_name'].'</td>
							<td>'.date('m/d/Y', strtotime($value['date_out'])).'</td>
							<td>'.$value['receiver_name'].'</td>
							<td>'.date('h:i A', strtotime($value['date_out'])).'</td>
							<td>'.$value['action_taken'].'<br>'.$value['remarks'].'</td>
						</tr>
						';
					}
					$i = 17 - count($trans_log);
					while ($i > 0) {
