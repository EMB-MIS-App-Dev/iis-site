...
?>
					...
					<div class="trans-layout card shadow mb-4">
						...
						<div class="card-body">
							...
							<div class="table-responsive">
								<table id="dttb_sample" class="table table-hover table-striped" width="100%" cellspacing="0">
									<thead>
										<tr>
											<th>no</th>
		...
		<script>
			...
			$(document).ready(function() {
			    ...
			    var table = $('#dttb_sample').DataTable({
        			...
        			"scrollX": true,
			        ajax: {
			            "url": "<?php echo base_url('Dms/Datatables/dttb_sample'); ?>",
			            "type": 'POST',
			    ...
			    $('#dttb_sample tbody').on( 'click', '#viewbtn', function () {
			        var data = table.row( $(this).parents('tr') ).data();
							Dms.view_transaction( data['trans_no'] );
				} );

				$('#dttb_sample tbody').on( 'click', '#prcbtn', function () {
			        var data = table.row( $(this).parents('tr') ).data();
