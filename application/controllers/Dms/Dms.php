...
<?php
  ...
  class Dms extends CI_Controller
  {
    ...
    function __construct()
    {
      ...
      $where['options']    = array('ao.userid' => $this->session->userdata('userid'));
      $globvar['options']  = $this->Embismodel->selectdata('acc_options AS ao', '', $where['options']);

      $where['func']      = array('af.userid' => $this->session->userdata('userid'), 'af.stat' => 1);
      ...
      $this->dmsdata = array(
        ...
        'user_options'  => $globvar['options'][0],
      );
    ...
    function _dms_update($content)
      ...
      if ( ! $this->session->userdata('trans_session')) {
        ...
        if(empty($trans_check[0]['trans_no']))
        {
          ...
          if($this->dmsdata['user_token'] == '515e12d4a186a84' || $this->dmsdata['user_token'] == '1255e154af92caff' || $this->session->userdata('superadmin_rights') == 'yes')
          {
    ...
    function all_transactions()
    {
      ...
      $this->_dms_view('all_transactions');
    }

    function inbox()
    {
      redirect(base_url('dms/documents/inbox'));
      $this->_dms_view('inbox');
    }

    function outbox()
    {
      $this->_dms_view('outbox');
    }
    ...
    function add_transaction()
    {
      ...
      $this->dmsdata['qr_code_image'] = $this->trans_qrcode();
    ...
    function create_transaction()
    {
      ...
      $qr_code_token = $this->trans_qrcode();
    ...
    function save_draft()
    {
      ...
      if($result)
      {
        ...
        $red_url = empty($red_url) ? 'drafts' : $red_url == 'all_transactions' ? 'all' : $red_url == 'draft'? 'drafts' : $red_url;

        redirect( base_url('dms/documents/'.$red_url));
    ...
    function process_transaction()
    {
      ...
      if($result)
      {
        ...
        $red_url = empty($red_url) ? 'outbox' : $red_url == 'all_transactions' ? 'all' : $red_url == 'draft'? 'drafts' : $red_url;
    ...
    function assigned_slctn()
    {
      ...
      $div_where = $this->db->where('adv.divno != 15 AND adv.office = "'.$this->session->userdata('office').'" AND (adv.type = "'.$reg_type.'" OR adv.type = "'.$this->session->userdata('region').'")');
    ...
    function receive_transaction()
    {
      ...
      {
        $res['error'] = 0;
        $res['site'] = base_url('dms/documents/inbox');
      }
    ...
    function select_region()
    {
      ...
      if(!empty($this->input->post('region_name'))) {
        ...
        $type2 = strtolower($this->input->post('region_name'));

        if($this->input->post('region_name') == $region_name || $this->session->userdata('func') == 'Director' || $this->session->userdata('func') == 'Assistant Director')
        {
          $div_restriction = '1';
        }
        else if($this->session->userdata('func') == 'Regional Director')
        {
          $div_restriction = 'axd.divno IN (1, 2, 6, 14, 17)';
        }
        else {
          $div_restriction = 'axd.divno IN (2, 6)';
        }

        $where['xdvsion'] = $this->db->where('axd.type IN("'.$type.'", "'.$type2.'") AND axd.divno != 15 AND axd.office = "EMB" AND '.$div_restriction);
      }
    ...
    function select_division()
    {
      ...
      if($region != 'CO') {
        $where['xsect'] = $this->db->where('axs.secno != 345 AND axs.divno = "'.$division_name.'" AND ( axs.region = "'.$region.'" OR axs.region = "R" ) AND axs.secno NOT IN ('.$xsct.') AND '.$sec_restriction);
      }
      else {
        $where['xsect'] = $this->db->where('axs.secno != 345 AND axs.divno = "'.$division_name.'" AND axs.region = "'.$region.'" AND '.$sec_restriction);
      }
    ...
    function run_select_division()
    {
      ...
      if($region != 'CO') {
        $where['xsect'] = $this->db->where('axs.secno != 345 AND axs.divno = "'.$division_name.'" AND ( axs.region = "'.$region.'" OR axs.region = "R" ) AND axs.secno NOT IN ('.$xsct.')');
      }
      else {
        $where['xsect'] = $this->db->where('axs.secno != 345 AND axs.divno = "'.$division_name.'" AND axs.region = "'.$region.'"');
      }
      $division = $this->Embismodel->selectdata('acc_xsect AS axs', '', '', $where['xsect']);
      echo json_encode($division);
    }
