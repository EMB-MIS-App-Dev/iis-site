				...
				<div class="col-xl-12 col-lg-12">
					<div class="trans-layout card shadow mb-4">
						...
						<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
							...
							<h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-tasks"></i> PERMIT - HAZWASTE</h6>
						...
						<div class="card-body">
							...
							<div class="table-responsive" style="zoom: 90%">
								<table id="hazwaste_table" class="table table-hover table-striped" width="100%" cellspacing="0">
									<thead>
										<tr>
											<th>no</th>
	...
	<div hidden id="haztype_selection">
		<select id="haz_slctd" class="form-control" name="hazwaste_slct" >
			<?php
				foreach ($system_type as $key => $value) {
					if(!empty($_SESSION['permit_type']) && $value['id'] == $_SESSION['permit_type'])
					{
						echo '<option selected value="'.$value['id'].'">'.$value['name'].'</option>';
					}
					else if(empty($_SESSION['permit_type']) && $value['id'] == 98)
					{
						echo '<option selected value="'.$value['id'].'">'.$value['name'].'</option>';
					}
					else {
		...
		<script>
			...
			$(document).ready(function() {
			    ...
			    var table = $('#hazwaste_table').DataTable({
				...
				$("div.type").html($('#haztype_selection').html());

		    $('#hazwaste_table tbody').on( 'click', '#viewbtn', function () {
			        var data = table.row( $(this).parents('tr') ).data();
							Dms.view_transaction( data['trans_no'] );
				} );

				$('#haz_slctd').change(function() {
          tableData.type = $('#haz_slctd').val();
					table.draw();
				} );
