...
class SSP {
    ...
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '')
    {
        ...
        $default_select="SELECT SQL_CALC_FOUND_ROWS ";
        ...
        if($joinQuery){
            ...
            $query =  "`".implode("`, `", SSP::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
       $having
        ...
        $db='';
        $query1 = $default_select.$query;
        $data = SSP::sql_exec1( $db, $bindings, $query1);

        // $query2 = "SELECT FOUND_ROWS() as `0`, ".$query;
        $resFilterLength = SSP::sql_exec1( $db,
            $query1, null, 1
        );
        ...
        $resTotalLength = SSP::sql_exec1( $db,
            "SELECT COUNT(`{$primaryKey}`) AS `0`
			 FROM   `$table`"
        );
    ...
    static function sql_exec1 ( $db="", $bindings, $sql=null, $type=null )
    {
        $ci =& get_instance();
        // Argument shifting
        if ( $sql === null ) {
