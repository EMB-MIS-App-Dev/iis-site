    ...
    function json_company(){
      ...
      if($api_key == $api_key_dev){
        ...
        if (isset($emb_id) && !is_null($emb_id)){
          ...
          // $whraf = $this->db->where('acr.deleted !=',0);
          ...
          $afltd = $this->Embismodel->selectdata('approved_client_req AS acr','CONCAT(UPPER(TRIM(first_name))," ",UPPER(TRIM(last_name))) AS name',$jnaf,$whraf);

          //(!empty($afltd[0]['name'])) ? $afltdarray = $afltd : $afltdarray = array();
          (count($afltd) > 0 ) ? $afltdarray = $afltd : $afltdarray = array();
          $array = array();
