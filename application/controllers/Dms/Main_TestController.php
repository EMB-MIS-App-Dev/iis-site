<?php
  ...
  */
  class Main_TestController extends CI_Controller
  {
    private $dmsdata;
    ...
    function index()
    {
      ...
      if($this->uri->segment(1) != 'test' && $this->uri->segment(2) != 'documents') {
    ...
    private function router() // MAINLY FOR URI SEGMENT CHECKING ONLY
    {
      switch ($this->uri->segment(3)) {
        ...
        case 'inbox123':
            $this->inbox123();
            $this->load->view('Dms/modals/main_modal.php');
...
// ----------------------------------------------------------------- VIEWS -------------------------------------------------------------------- //
    ...
    private function all_transactions()
    {
      ...
      $this->load->view('Dms/all_transactions_test', $data);
    }

    private function sample_all()
    {
      $data = array(
        'user_region'  => $this->dmsdata['user_region'],
        'user_token'   => $this->dmsdata['user_token'],
      ...
      $this->load->view('Dms/all_transactions1', $data);
    }
    ...
    private function inbox123()
    {
      ...
      $data['division'] = $this->Embismodel->selectdata( 'acc_xdvsion', '*', array('type' => $div_uloctype, 'office' => 'EMB') );
      $data['action'] = $this->Embismodel->selectdata('er_action', '*');
      $this->load->view('Dms/inbox2', $data);
    }
