<?php
  ...
  */
  class Test_Process extends CI_Controller
  {
    private $dmsdata;
    ...
    function index()
    {
      // if($this->uri->segment(1) != 'dms' && $this->uri->segment(2) != 'documents' && $this->uri->segment(3) != 'test') {
      //   redirect(base_url('error_404'));
      // }
      // if($this->uri->segment(2) === 'documents' && !empty($this->uri->segment(3)) && $this->uri->segment(3) == 'test') { // ADD XSS_CLEAN
    ...
    function test_selection()
    {
      ...
      $data['user']['function'] = $this->Embismodel->selectdata('acc_function', '*', array('userid' => $data['user']['id']) );
        ...
        $func = $this->Embismodel->selectdata('acc_function', 'func', array('stat' => 1, 'userid' => $data['user']['id']));
      ...
      // ADD SWITCH HERE
      switch ($data['user']['function'][0]['func']) {
        case 'Administrator':
        case 'Director':
      ...
      $this->load->view('Dms/func/test_1', $data);
    ...
    private function router() // MAINLY FOR URI SEGMENT CHECKING ONLY
    {
      switch ($this->uri->segment(3)) {
        case 'test': // SEPARATE TO ANOTHER DMS CONTROLLER FILE
          if($this->uri->segment(4) == 'process')
          {
            if(!empty($this->uri->segment(5)) && !empty($this->uri->segment(6)) && !empty($this->uri->segment(7)) && !empty($this->uri->segment(8))) {
...
// ----------------------------------------------------------------- VIEWS -------------------------------------------------------------------- //
    ...
    private function route_transaction()
    {
      $data = array(
        ...
        'trans_no'        => $this->uri->segment(6),
        'main_multi_cntr' => $this->uri->segment(7),
        'multi_cntr'      => $this->uri->segment(8),
        'user_region'     => $this->dmsdata['user_region'],
        'user_token'      => $this->dmsdata['user_token'],
      );
      $data['enc_key'] =  $this->encrypt->encode( trim($data['entry'].';'.$data['trans_no'].';'.$data['main_multi_cntr'].';'.$data['multi_cntr']) );
      $data['user'] = $this->dmsdata['user'];
      ...
      if($data['entry'] != 1)
      {
        ...
        for ($index=0; $index < count($data['multicnt'])+1; $index++)  {
          $history = array(
            'where' => array(
              'trans_no'          => $data['trans_no'],
              'main_multi_cntr'   => $index<1 ? '': $data['multicnt'][$index-1],
        ...
        foreach ($result['trans_history'][0] as $key => $value)
        {
          ...
          $result['attachments'][0][0] = $this->Embismodel->selectdata( 'er_attachments', '*', $history['where'], $history['order'] );
          // XDIVISION AND XSECT QUERIES FOR SENDER AND RECEIVER
          $sender['div'] = $this->Embismodel->selectdata( 'acc_xdvsion', 'divcode', array( 'divno' => $value['sender_divno'], 'office' => 'EMB' ) )[0];
          $sender['sec'] = $this->Embismodel->selectdata( 'acc_xsect', 'secode', array('secno' => $value['sender_secno']) )[0];
          $receiver['div'] = $this->Embismodel->selectdata( 'acc_xdvsion', 'divcode', array( 'divno' => $value['receiver_divno'], 'office' => 'EMB' ) )[0];
          ...
          $result['ds_sender'][0][0] = (!empty($value['sender_region'])) ? $value['sender_region'] : '';
          $result['ds_sender'][0][0] .= (!empty($sender['div'])) ?  '|'.$sender['div']['divcode'] : '';
          $result['ds_sender'][0][0] .= (!empty($sender['sec'])) ? '|'.$sender['sec']['secode'] : '';
          // RECEIVER DIVISION AND SECTION CODES
          $result['ds_receiver'][0][0] = (!empty($value['receiver_region'])) ? $value['receiver_region'] : '';
          $result['ds_receiver'][0][0] .= (!empty($receiver['div'])) ?  '|'.$receiver['div']['divcode'] : '';
          $result['ds_receiver'][0][0] .= (!empty($receiver['sec'])) ? '|'.$receiver['sec']['secode'] : '';
        }
      }
      return $result;
    }
...
// ------------------------------------------- FORM DATA ------------------------------------------ //
    ...
    private function process_transaction()
    {
      ...
      */
      ...
      $asgnto_fail = array(
        'alert_data'  => array( 'title' => 'ERROR', 'text' => 'It Appears that the selected Personnel has either an Inactive/Retired Status, Still Awaiting Approval, Deleted, or Has Suddenly Change Division/Section. Please try again. <br>If this error persists, Please Contact Administrator. [USR-FAIL-PRC]', 'type' => 'error' ),
        'redirect'    => base_url('dms/documents/process/'.$enc_data[0].'/'.$enc_data[1].'/'.$this->_is_empty($enc_data[2], 1).'/'.$enc_data[3]),
      );
      ...
      if(empty($this->input->post('asgnto_multiple'))) { // MULTIPLE
        ...
        // EMPTY RESULT
        if(!$func_check) { $this->_alert($asgnto_fail['alert_data'], $asgnto_fail['redirect']); exit; }
      }
      else {
        if(empty($post['multislct_prsnl']) || count($post['multislct_prsnl']) < 2) { $this->_alert($multi_asgnto_fail['alert_data'], $multi_asgnto_fail['redirect']); exit; }
      }
      /* START OF PROCESSING */
        ...
        foreach ($post['multislct_prsnl'] as $key => $value) {
          ...
          */
          ...
          $ins_mtrns_etrlog = array(
            'trans_no'            => $data['trans_no'],
            // 'main_route_order'    => $data['trnsctn'][0]['main_route_order']+1,
            'route_order'	        => 1,
            'multiprc'	          => 0,
            'main_multi_cntr'	    => trim($this->_is_empty($data['main_multi_cntr'], $data['multi_cntr'], '-')),
            'multi_cntr'          => $key+1,
