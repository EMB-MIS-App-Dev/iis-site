...
class ServersideTicket extends CI_Controller

{
  ...
  public function index(){
      ...
      // Include SQL query processing class
        ...
        $joinQuery  = "FROM `embis`.`to_trans` AS `tt` LEFT JOIN `embis`.`to_ticket_request` AS `ttr` ON `ttr`.`er_no`=`tt`.`er_no`
          LEFT JOIN `embis`.`er_transactions` AS `et` ON `et`.`trans_no`=`tt`.`er_no`";
        $extraWhere = "`tt`.`region` = '".$region."'  AND `tt`.`travel_type` = 'Air' AND (`tt`.`status` = 'Approved' OR `tt`.`status` = 'Signed Document') AND `et`.`status` != 0 AND `et`.`action_taken` != 'Issued Ticket'";
        $groupBy    = null;
        $having     = null;
  ...
  public function all_issued_ticket(){
      ...
      // Include SQL query processing class
        ...
        $joinQuery  = "FROM `embis`.`to_trans` AS `tt` LEFT JOIN `embis`.`to_ticket_request` AS `ttr` ON `ttr`.`er_no`=`tt`.`er_no`
          LEFT JOIN `embis`.`er_transactions` AS `et` ON `et`.`trans_no`=`tt`.`er_no`";
        $extraWhere = "`tt`.`region` = '".$region."'  AND `tt`.`travel_type` != 'Land' AND (`tt`.`status` = 'Approved' OR `tt`.`status` = 'Signed Document') AND `et`.`status` != 0 AND `ttr`.`cnt` != ''";
        $groupBy    = null;
        $having     = null;
