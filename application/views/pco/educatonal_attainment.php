          ...
          <div class="col-md-12 card shadow mb-4 border-left-primary">
            ...
            <div class="card-body" id="educ_attain">
              <fieldset>
                ...
                <button type="button" class="btn btn-primary" data-target=".addEducAttainModal" data-toggle="modal"> <i class="fas fa-plus"></i> Add New Row </button> <br><br>
                <input type="text" name="educ_atn_chkempty" value="<?=$educ_atn_chkempty?>"/>

                <div class="table-responsive" >
                  <table id="educ_attain_table" class="table table-bordered ">
                    <thead>
                      <tr>
                        <th></th>
...
<script>
  ...
  $(document).ready(function(){
  		...
  		var table = $('#educ_attain_table').DataTable({
  	    order: [[0, "desc"]],
        lengthMenu: [[10, 25, 50, -1], [10, 25, 50, "All"]],
  			...
  			responsive: true,
  			ajax: {
  					"url": "<?php echo base_url('Pco/Datatables/educatonal_attainment'); ?>",
  					"type": 'POST',
  	      ...
  	      { "data": "address" },
  	      { "data": "inclusive_date" },
  	      { "data": "degree_units_earned" },
  	      { "data": "cnt", "visible": false, "searchable": false },
  	      {
            "sortable": false,
            "render": function(data, type, row, meta) {
                data = "<button type='button' id='editbtn' class='btn btn-warning btn-sm waves-effect waves-light' title='Edit' data-target='.editEducAttainModal' data-toggle='modal'><i class='far fa-edit'></i></button>&nbsp;";
                return data;
            }
  	      }
  	    ]
      ...
      $('input[name="received_prof_license"]', '#educational_attainment_form').change(function(){
  	  ...
  	  $('#educ_attain_table tbody').on( 'click', '#editbtn', function () {
  	    var data = table.row( $(this).parents('tr') ).data();
        $.ajax({
           url: '<?=base_url("Pco/Data/edit_educatn_html")?>',
           method: 'POST',
           data: { user_id: data['user_id'], cnt: data['cnt'] },
           success: function(data) {
             $('#editeducatn_div').html(data);
           }
        });
  	  });


  		// $('button[name="add_est_details_btn"]').click(function() {
  		// 	table.ajax.reload();
      //   var frm = $('form#estdetailsform');
      ...
      // $('form#estdetailsform').submit( function() {
        ...
        var frm = $('#estdetailsform');
            ...
            $('fieldset#fieldset').prop('disabled', true);
          }
    		});
            ...
            table.ajax.reload();
            _this.attr('disabled',false);
            $('.addEducAttainModal').modal("hide");
            $('fieldset#fieldset').prop('disabled', false);
    			}
    		});
      ...
      $('button[name="edit_est_details_btn"]').click(function() {
      // $('form#estdetailsform').submit( function() {
        var _this = $(this);
        var frm = $('#editestdetailsform');
            ...
            table.ajax.reload();
            _this.attr('disabled',false);
            $('.editEducAttainModal').modal("hide");
            $('fieldset#fieldset').prop('disabled', false);
    			}
    		});
