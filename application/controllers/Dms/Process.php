  ...
  */
  class Process extends CI_Controller
  {
    ...
    function index()
    {
      ...
      if($this->uri->segment(1) != 'dms' && $this->uri->segment(2) != 'documents' && $this->uri->segment(3) != 'process') {
        redirect(base_url('error_404'));
      }
      if($this->uri->segment(2) === 'documents' && !empty($this->uri->segment(3)) && $this->uri->segment(3) == 'process') { // ADD XSS_CLEAN
        $this->router();
      }
      else {
        redirect(base_url('dms/documents/all'));
...
// ------------------------------------------- FORM VALIDATIONS ------------------------------------------ //
    private function validate_form($site="", $data="")
    {
      ...
      if(isset($_POST['process_transaction']))
      {
        ...
        //   $this->form_validation->set_rules('courier_type', 'asd', 'required_courier');
        // }
