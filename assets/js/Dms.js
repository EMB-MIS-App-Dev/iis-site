...
var Dms = {
  ...
  view_transaction123: function(trans_no)
  {
    $.ajax({
       url: Dms.base_url + 'Test_Process/view_transaction',
       method: 'POST',
  ...
  view_transaction: function(trans_no, multiprc='')
  {
    $.ajax({
  ...
  trans_view: function(trans_no, multiprc='')
  {
    $.ajax({
       url: Dms.base_url + 'Modal_Data/view_transaction',
       method: 'POST',
       data: { trans_no : trans_no },
       success: function(data) {
         $('#view_transaction_modal').html(data);
  ...
  view_transaction11: function(trans_no, main_multi_cntr, multiprc)
  {
    $.ajax({
       url: Dms.base_url + 'Dms/view_transaction11',
       method: 'POST',
       data: { trans_no : trans_no, main_multi_cntr : main_multi_cntr, multiprc : multiprc },
       success: function(data) {
         $('#view_transaction_modal').html(data);
       }
    });
  ...
  bulk_download: function(val='')
  {
    console.log(val);
    $.ajax({
       url: Dms.base_url + 'Dms/bulk_download',
       method: 'POST',
       data: { val : val },
       success: function(data) {
         console.log('success');
  ...
  add_trans_company: function(transaction_id, staff_id='', page_name='')
  {
    $.ajax({
         url: Dms.base_url + 'Dms/add_trans_company',
         type: 'POST',
         data: { transaction_id: transaction_id, staff_id: staff_id, page_name: page_name },
         success: function(result) {
             $('#addcompany_div').html(result);
         }
     });
   },

  add_company_modal: function()
  {
   $.ajax({
        url: Dms.base_url + 'Ajax_Data/add_company_modal',
        type: 'POST',
        success: function(result) {
            $('div.transferTransByCompanyModal div.modal-body').html(result);
        }
    });
  },
  ...
  {
    // $('#personnel_id').attr('required', true);
    $('#courier_type_slct').attr('required', false);
    if(_.contains(['17', '18', '19', '24'], trns_stat)) { // , '27'
      $('#asgnprsnl_div').hide();
      $('#personnel_id').attr('required', false);
      if(trns_stat == 19) {
        $('#couriertype_div').show();
        $('#courier_type_slct').attr('required', true);
      }
      else {
  ...
  select_section: function (section_name, _this = '')
  {
    ...
    $.ajax({
          ...
          success:function(data)
          {
            ...
            if(section_name){
              for( var i=0; i < data.length; i++){
                  if(data[i]['token'] == '5715e58cf2dac134') {
                    html +='<option value="'+data[i]['token']+'" disabled class="asgn-on-leave">'+data[i]['fname']+' '+data[i]['sname']+' '+data[i]['suffix']+'<span > [ON-LEAVE]</span> </option>';
                  }
                  else if(data[i]['token'] == '4635e4286ea323ec') {
                    html +='<option value="'+data[i]['token']+'" disabled class="asgn-on-leave">'+data[i]['fname']+' '+data[i]['sname']+' '+data[i]['suffix']+'<span > [PAUSED-ROUTING]</span> </option>';
                  }
                  else {
        ...
        if(data.length > 1)
        {
          ...
          for (var i = 0; i < data.length; i++) {
            if(data[i]['receiver'])
            {
              ...
              if(data[i]['remarks'])
              {
                rem_display = ' [with special remarks]';
                var rem = data[i]['remarks'];
                remark = rem.replace(/"/g, '\\"');

              }
              html += '<input type="hidden" name="multislct_prsnl[]" value="'+data[i]['region']+';'+data[i]['division']+';'+data[i]['section']+';'+data[i]['receiver']+';'+data[i]['name']+';'+remark+'" />';
              html += data[i]['prsnl']+''+rem_display;
              if(i < data.length) {
                html += '<br />';
              }
            }
