<?php
  ...
  class Universe extends CI_Controller
  {
    ...
    function __construct()
    {
      ...
      if(!empty($this->input->get('ufunc'))) {
        $globvar2['func'] = $this->Embismodel->selectdata( 'acc_function AS af', '*', array('af.userid' => $this->session->userdata('userid'), 'af.stat' => 1, 'af.cnt' => $this->input->get('ufunc')) );
        if($globvar2['func']) {
          redirect('Universe/Universe/accomp_all');
        }
        $globvar['func'] = $globvar2['func'];
      }
    ...
    function accomp_all()
    {
      ...
      $this->univdata['func_list'] = $this->Embismodel->selectdata( 'acc_function', '*', array('userid' => $this->session->userdata('userid'), 'stat' => 1) );
    ...
    function accomp_filter()
    {
      ...
      if($this->input->post('type') == 'filter') {
        $data['filter'] = array(
          ...
          'personnel'   => (!empty($this->input->post('personnel'))) ? $this->input->post('personnel') : '',
