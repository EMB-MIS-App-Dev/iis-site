  ...
  class Dms extends CI_Controller
  {
    ...
    function validate_form($site)
    {
      if(isset($_POST['process_transaction']))
      {
        ...
        if($this->input->post('status') == '24' && ($this->dmsdata['user_func']['secno'] == '77') ) {
    ...
    function process_transaction()
    {
      ...
      $trans_vars = array(
        ...
        'set'   => array(
          ...
          'et.sender_id'          => $data['user_token'],
          'et.sender_name'        => $data['snder'],
          'et.receiver_division'  => (!empty($data['post_data']['division'])) ? $data['post_data']['division'] : $data['cred_snder'][0]['division'],
          'et.receiver_section'   => (!empty($data['post_data']['section'])) ? $data['post_data']['section'] : $data['cred_snder'][0]['section'],
    ...
    function fileUpload()
    {
      ...
      $att_token = $data['trans_region'].date('Y').'-FT'.$data['route_order'].'N'.$att_token1.'-File_'.($ea_q[0]['max_fileid']+1);
    ...
    function select_division()
    {
      ...
      $div_where = array(
        'axs.divno'   => $division_name,
        'axs.region'  => $region,
      );
      $division = $this->Embismodel->selectdata('acc_xsect AS axs', '', $div_where);
      echo json_encode($division);
    }
