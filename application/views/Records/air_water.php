				...
				<div class="col-xl-12 col-lg-12">
					<div class="trans-layout card shadow mb-4">
						...
						<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
							...
							<h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-tasks"></i> PERMIT - AIR/WATER</h6>
						...
						<div class="card-body">
							...
							<div class="table-responsive" style="zoom: 90%">
								<table id="air_water_table" class="table table-hover table-striped" width="100%" cellspacing="0">
									<thead>
										<tr>
											<th>no</th>
	...
	<div hidden id="air_watertype_selection">
		<select id="air_water_slctd" class="form-control" name="air_water_slct" >
			<?php
		...
		<script>
			...
			$(document).ready(function() {
					var tableData = {
						'type': $('#air_water_slctd').val(),
						'start_date': $('input[name="start_date"]').val(),
						'end_date': $('input[name="end_date"]').val(),
					};
			    var table = $('#air_water_table').DataTable({
				...
				$("div.type").html($('#air_watertype_selection').html());

		    $('#air_water_table tbody').on( 'click', '#viewbtn', function () {
			        var data = table.row( $(this).parents('tr') ).data();
							Dms.view_transaction( data['trans_no'] );
				} );

				$('#air_water_slctd').change(function() {
          tableData.type = $('#air_water_slctd').val();
					table.draw();
				} );
